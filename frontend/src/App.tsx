...
import { Box, Container, Typography, Paper, Stack, TextField, Button,
  ...
  Stepper, Step, StepLabel, TablePagination, Chip, Divider } from '@mui/material';
...
export default function App() {
  ...
  const handleRequestMyOtp = async () => {
    setMyOtpError('');
    setMySendingOtp(true);
    try {
      const res = await fetch(`${API}/api/bookings/otp`, {
        method: 'POST',
      ...
      setMyOtpSent(true);
      setMyOtpCooldown(body.secondsRemaining ?? 600);
      if (body.secondsRemaining) {
        setMyError(`You already have an active OTP valid for ${Math.ceil(body.secondsRemaining / 60)} more minute(s).`);
      }
      setMyBookings([]);
      setMySearched(false);
    } catch {
      setMyError('Network error');
    } finally {
      setMySendingOtp(false);
    }
  };
  ...
  const handleMyBookings = async (page = 0) => {
    ...
    try {
      ...
      setMyError('Failed to fetch bookings');
    }
  };
  ...
  const handleRequestBookingOtp = async () => {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok) {
        setOtpError(body.error ?? 'Failed to send OTP');
        return;
      }
      setBookingOtpCooldown(body.secondsRemaining ?? 600);
      setStep(2);
    } catch {
    ...
    <React.Fragment>
      ...
      <Box sx={{ bgcolor: '#e3f2fd', minHeight: '100vh' }}>
        <Container maxWidth="md">
            ...
            {!myOtpSent ? (
              <Stack direction="row" spacing={2} alignItems="center">
                <TextField label="Your Email" type="email" value={myEmail} autoComplete="off"
                  ...
                  sx={{ flex: 1 }} />
                <Button variant="contained" disabled={!myEmail.includes('@') || mySendingOtp}
                  onClick={handleRequestMyOtp}>
              ...
              <Stack spacing={2}>
                <Typography variant="body2" color="text.secondary">
                  OTP sent to <strong>{myEmail}</strong>. Check your email (or server console in dev).
                </Typography>
                <Stack direction="row" spacing={2} alignItems="center">
                  <TextField label="Enter OTP" value={myOtp}
                    ...
                    sx={{ width: 160 }} />
                  <Button variant="contained" disabled={myOtp.length !== 6}
                    onClick={() => handleMyBookings(0)}>View Bookings</Button>
                  <Button variant="text" size="small" disabled={otpResending || myOtpCooldown > 0}
                    onClick={handleResendMyOtp}>
          ...
          {mySearched && myBookings.length === 0 && (
            <Typography color="text.secondary" sx={{ textAlign: 'center', mt: 2, mb: 4 }}>
              No bookings found for {myEmail}.
            </Typography>
          )}
          ...
          {mySearched && myTotal > 10 && (
            <TablePagination
              ...
              count={myTotal}
              page={myPage}
              onPageChange={(_, p) => handleMyBookings(p)}
              rowsPerPage={10}
              rowsPerPageOptions={[10]}
            />
          )}
      ...
      <Dialog open={!!selectedRoom} onClose={handleCloseDialog} fullWidth maxWidth="sm">
          ...
          {step === 2 && (
            <Stack spacing={2}>
              ...
              <TextField
                ...
                error={!!otpError}
                helperText={otpError || `Enter the 6-digit OTP sent to ${customer.email}`}
                slotProps={{ htmlInput: { maxLength: 6 } }}
              />
              <Button variant="text" size="small" sx={{ alignSelf: 'flex-start' }}
                disabled={bookingOtpCooldown > 0}
                onClick={handleRequestBookingOtp}>
        ...
        <DialogActions>
          ...
          {step === 0 && (
            <Button variant="contained" disabled={!customer.name || !customer.email.includes('@')} onClick={handleNext}>Next</Button>
          )}
          {step === 1 && (
            <Button variant="contained" onClick={handleRequestBookingOtp}>Continue with Vipps</Button>
          )}
          {step === 2 && (
            <Button variant="contained" disabled={otp.length !== 6 || bookingLoading} onClick={handleCompletePayment}>
